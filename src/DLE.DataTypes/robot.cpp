...
void tRobotGunInfo::Read (IFileManager* fp, int nField) 
{
if (nField == 0)
	fp->Read (points);
else
...
void tRobotGunInfo::Write (IFileManager* fp, int nField) 
{
if (nField == 0)
	fp->Write (points);
else
...
void tRobotExplInfo::Read (IFileManager* fp) 
{
nClip = fp->ReadInt16 ();
nSound = fp->ReadInt16 ();
}

void tRobotExplInfo::Write (IFileManager* fp) 
{
fp->Write (nClip);
fp->Write (nSound);
}
...
void tRobotContentsInfo::Read (IFileManager* fp) 
{
id = fp->ReadSByte ();
count = fp->ReadSByte ();
prob = fp->ReadSByte ();
...
void tRobotContentsInfo::Write (IFileManager* fp) 
{
fp->Write (id);
fp->Write (count);
fp->Write (prob);
...
void tRobotSoundInfo::Read (IFileManager* fp) 
{
see = fp->ReadUByte ();
attack = fp->ReadUByte ();
claw = fp->ReadUByte ();
...
void tRobotSoundInfo::Write (IFileManager* fp) 
{
fp->Write (see);
fp->Write (attack);
fp->Write (claw);
...
void tRobotCombatInfo::Read (IFileManager* fp, int nField) 
{
	switch (nField) {
		case 0:
...
void tRobotCombatInfo::Write (IFileManager* fp, int nField) {
	switch (nField) {
		case 0:
...
void CRobotInfo::Read (IFileManager* fp, int version, bool bFlag) 
{ 
	int i, j;
...
void CRobotInfo::Write (IFileManager* fp, int version, bool bFlag) 
{
	int i, j;
