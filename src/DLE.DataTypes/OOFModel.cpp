...
static sbyte OOF_ReadByte (IFileManager& fp, const char *pszIdent)
{
...
static int OOF_ReadInt (IFileManager& fp, const char *pszIdent)
{
int i = fp.ReadInt32 ();
return i;
}
...
static float OOF_ReadFloat (IFileManager& fp, const char *pszIdent)
{
...
static void OOF_ReadVector (IFileManager& fp, CFloatVector *pv, const char *pszIdent)
{
pv->v.x = fp.ReadFloat ();
pv->v.y = fp.ReadFloat ();
pv->v.z = fp.ReadFloat ();
...
static char *OOF_ReadString (IFileManager& fp, const char *pszIdent)
{
	char	*psz;
	int	l;
...
static int OOF_ReadIntList (IFileManager& fp, CDynamicArray<int>& list)
{
	uint	i;
	char	szId [20] = "";
...
static bool OOF_ReadVertList (IFileManager& fp, CDynamicArray<CFloatVector>& list, int nVerts, CFloatVector *pvMin, CFloatVector *pvMax)
{
	char	szId [20] = "";
...
int OOF::CFrameInfo::Read (IFileManager& fp, CModel* po, int bTimed)
{
nIndent += 2;
if (bTimed) {
...
int CRotFrame::Read (IFileManager& fp, int bTimed)
{
	float	fMag;
...
int CRotAnim::Read (IFileManager& fp, CModel* po, int bTimed)
{
...
int CPosFrame::Read (IFileManager& fp, int bTimed)
{
nIndent += 2;
if (bTimed)
	m_nStartTime = OOF_ReadInt (fp, "nStartTime");
...
int CPosAnim::Read (IFileManager& fp, CModel* po, int bTimed)
{
if (!this->CFrameInfo::Read (fp, po, bTimed))
	return 0;
if (bTimed &&
...
int CSpecialPoint::Read (IFileManager& fp)
{
Init ();
nIndent += 2;
...
int CSpecialList::Read (IFileManager& fp)
{
	uint	i;
...
int OOF::CPoint::Read (IFileManager& fp, int bParent)
{
nIndent += 2;
...
int CPointList::Read (IFileManager& fp, int bParent, int nSize)
{
	int	i;
...
int CAttachList::Read (IFileManager& fp)
{
	int	i;
...
int CAttachPoint::Read (IFileManager& fp)
{
...
int CAttachList::ReadNormals (IFileManager& fp)
{
	uint	i;
...
int CBattery::Read (IFileManager& fp)
{
nIndent += 2;
...
int CArmament::Read (IFileManager& fp)
{
	int	l;
...
int CFaceVert::Read (IFileManager& fp, int bFlipV)
{
nIndent += 2;
...
int CFace::Read (IFileManager& fp, CSubModel *pso, CFaceVert *pfv, int bFlipV)
{
	int	i, v0 = 0;
	CEdge	e;
...
if (!_stricmp (command, "$fov=")) {
	...
	sscanf_s(data, " %f, %f, %f", &fov_angle, &turret_spr, &reactionTime);
...
int CSubModel::Read (IFileManager& fp, CModel* po, int bFlipV)
{
	int				h, i;
#if OOF_MEM_OPT
...
int CModel::ReadTextures (IFileManager& fp)
{
	int			i;
	char*			pszName;
...
for (i = 0; i < m_textures.Count (); i++) {
	...
	char szFile [256];
	sprintf_s (szFile, "%s\\%s", m_folder, pszName);
	if (!strrchr (szFile, '.'))
		strcat_s (szFile, ".tga");
...
int CModel::ReadInfo (IFileManager& fp)
{
nIndent += 2;
