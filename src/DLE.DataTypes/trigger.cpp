...
short CTriggerTargets::Add (const CSideKey& key) 
{
...
int CTriggerTargets::Find (const CSideKey& key) 
{ 
...
void CTriggerTargets::Read (IFileManager* fp) 
{
	int i;
...
void CTriggerTargets::Write (IFileManager* fp) 
{
	int i;

for (i = 0; i < MAX_TRIGGER_TARGETS; i++)
	fp->WriteInt16 ((short) (((m_targets [i].m_nSegment < 0) || (m_targets [i].m_nSide <= 0)) ? m_targets [i].m_nSegment : g_data.segmentManager->Segment (m_targets [i].m_nSegment)->Index ()));
for (i = 0; i < MAX_TRIGGER_TARGETS; i++)
...
void CTrigger::Read (IFileManager* fp, bool bObjTrigger)
{
if (g_data.IsD2File ()) {
	...
	if ((g_data.LevelVersion () < 21) && (m_info.type == TT_EXIT))
		m_info.value = 0;
	if ((g_data.FileVersion () < 39) && (m_info.type == TT_MASTER))
		m_info.value = 0;
...
void CTrigger::Write (IFileManager* fp, bool bObjTrigger)
{
if (g_data.IsD2File ()) {
...
void CReactorTrigger::Read (IFileManager* fp, bool bFlag)
{
m_count = char (fp->ReadInt16 ());
#if 1
this->CTriggerTargets::Read (fp);
...
void CReactorTrigger::Write (IFileManager* fp, bool bFlag)
{
fp->Write (m_count);
#if 1
this->CTriggerTargets::Write (fp);
