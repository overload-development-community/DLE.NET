...
m_info.flags	= 0;
m_info.nSegment = g_data.currentSelection->SegmentId ();
...
int CGameObject::DrawArrow (IRenderer& renderer, int bCurrent)
{
#define ARROW_LINES 6
	...
	IViewMatrix*	viewMatrix = renderer.ViewMatrix ();
...
if (bOrtho) {
	...
	renderer.BeginRender ();
	renderer.SetLineWidth (bCurrent ? 6.0f : 3.0f);
...
bool CGameObject::DrawSprite (IRenderer& renderer)
{
...
void CGameObject::DrawHighlight (IRenderer& renderer, int bCurrent)
{
	IViewMatrix*	viewMatrix = renderer.ViewMatrix ();
	bool				bOrtho = renderer.Perspective () == 0;
	CVertex			center;
...
if (bOrtho) {
	...
	d = int (HasPolyModel() ? double (g_data.modelManager->ScreenRad ()) * 1.1 : 2000 / center.m_view.v.z);
	}
else
...
bool CGameObject::IsInView (IRenderer& renderer, bool wholeObject)
{
	bool result = true;
