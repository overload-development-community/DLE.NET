...
// strtok needs replacing to fix this properly
#pragma warning(disable:4996)

static char	szLine [1024];
static char	szLineBackup [1024];
static int nLine = 0;
static IFileManager *aseFile = null;
...
static void ReadVector (IFileManager& fp, CDoubleVector *pv)
{
#if ASE_ROTATE_MODEL
	pv->v.x = FloatTok (" \t");
	pv->v.z = FloatTok (" \t");
...
static char* ReadLine (IFileManager& fp)
{
while (!fp.EoF ()) {
...
int CSubModel::ReadNode (IFileManager& fp)
{
if (CharTok (" \t") != '{')
...
int CSubModel::ReadMeshVertexList (IFileManager& fp)
{
	CVertex*	pv;
	int		i;
...
int CSubModel::ReadMeshFaceList (IFileManager& fp)
{
	CFace	*pf;
	int	i;
...
int CSubModel::ReadVertexTexCoord (IFileManager& fp)
{
if (CharTok (" \t") != '{')
	return CModel::Error ("syntax error");
while ((pszToken = ReadLine (fp))) {
...
int CSubModel::ReadFaceTexCoord (IFileManager& fp)
{
if (CharTok (" \t") != '{')
	return CModel::Error ("syntax error");
while ((pszToken = ReadLine (fp))) {
...
int CSubModel::ReadMeshNormals (IFileManager& fp)
{
	CFace*	pf;
	CVertex*	pv;
	int		i;
...
int CSubModel::ReadMesh (IFileManager& fp, int& nFaces, int& nVerts)
{
if (CharTok (" \t") != '{')
	return CModel::Error ("syntax error");
while ((pszToken = ReadLine (fp))) {
...
int CSubModel::Read (IFileManager& fp, int& nFaces, int& nVerts)
{
while ((pszToken = ReadLine (fp))) {
	if (*pszToken == '}')
		return 1;
...
int CModel::ReadTexture (IFileManager& fp, int nTexture)
{
...
int CModel::ReadOpacity (IFileManager& fp, int nTexture)
{
	CTexture& tex = m_textures.Texture (nTexture);
...
int CModel::ReadMaterial (IFileManager& fp)
{
int i = IntTok (" \t");
...
int CModel::ReadMaterialList (IFileManager& fp)
{
if (CharTok (" \t") != '{')
	return CModel::Error ("syntax error");
...
int CModel::ReadSubModel (IFileManager& fp)
{
	CSubModel	*psm;
...
int CSubModel::SaveBinary (IFileManager& fp)
{
...
int CSubModel::ReadBinary (IFileManager& fp)
{
m_next = null;
