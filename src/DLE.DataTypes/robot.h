...
typedef struct tRobotGunInfo {
	...
	void Read (IFileManager* fp, int nField);
	void Write (IFileManager* fp, int nField);
} tRobotGunInfo;
...
class CRobotInfo : public CGameItem {
	...
	public:
		void Read (IFileManager* fp, int version = 0, bool bFlag = false);

		void Write (IFileManager* fp, int version = 0, bool bFlag = false);
