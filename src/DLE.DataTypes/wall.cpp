...
{
g_data.undoManager->Begin (__FUNCTION__, udWalls);
// define new pWall
*((CSideKey*) this) = key;
//m_nSegment = nSegment;
//m_nSide = nSide;
...
switch (type) {
	...
	case WALL_ILLUSION:
		...
		if (nTexture == -1)
			g_data.segmentManager->SetTextures (key, g_data.IsD1File () ? 328 : 353, 0); // energy
		else if (nClip == -2)
			g_data.segmentManager->SetTextures (key, 0, nTexture);
		else
			g_data.segmentManager->SetTextures (key, nTexture, 0);
		break;
	...
	case WALL_OVERLAY: // d2 only
		...
		m_info.hps = 0;
		// define box01a
		g_data.segmentManager->SetTextures (key, -1, 414);
		break;
...
g_data.segmentManager->Segment (key.m_nSegment)->SetUV (key.m_nSide, 0.0, 0.0);
...
if (IsDoor ()) {
	...
	g_data.undoManager->Unroll (__FUNCTION__);
	return;
	}
g_data.undoManager->End (__FUNCTION__);
...
void CWall::Read (IFileManager* fp, bool bFlag)
{
...
m_info.flags = ushort ((g_data.FileVersion () < 37) ? fp->ReadSByte () : fp->ReadInt16 ());
...
void CWall::Write (IFileManager* fp, bool bFlag)
{
fp->WriteInt32 ((m_nSegment < 0) ? m_nSegment : g_data.segmentManager->Segment (m_nSegment)->Index ());
...
if (m_info.nTrigger == NO_TRIGGER)
	...
	fp->WriteByte ((ubyte) g_data.triggerManager->Trigger (m_info.nTrigger)->Index ());
...
{
return g_data.segmentManager->Side (*this);
}
...
{
return g_data.triggerManager->Trigger (m_info.nTrigger);
}
...
switch (EditType ()) {
	case opAdd:
		g_data.wallManager->Remove (Index ());
		break;
	case opDelete:
		g_data.wallManager->Add (true);
		// fall through
	case opModify:
		*GetParent () = *this;
		break;
...
void CDoor::Read (IFileManager* fp, bool bFlag)
{
...
void CDoor::Write (IFileManager* fp, bool bFlag)
{
