...
void CSide::Read (IFileManager* fp, bool bTextured)
{
if (g_data.LevelVersion () > 24) {
	for (int i = 0; i < 4; i++)
...
if (bTextured) {
...
#if 1 //DBG
		if ((m_info.nOvlTex & TEXTURE_MASK) > g_data.textureManager->MaxTextures ())
			m_info.nOvlTex = 0;
#endif
		}
	else
	...
	if (m_info.nBaseTex > g_data.textureManager->MaxTextures ())
		m_info.nBaseTex = 0;
	for (int i = 0; i < 4; i++)
...
void CSide::Write (IFileManager* fp, bool bTextured)
{
if (g_data.LevelVersion () > 24) {
	for (int i = 0; i < 4; i++)
...
{
g_data.textureManager->Load (BaseTex (), OvlTex (0));
}
...
{
uint scale = (uint)g_data.textureManager->Scale (g_data.FileType (), nTexture);
for (int i = 0; i < 4; i++) {
...
{ 
return ((short) m_info.nChild < 0) ? null: g_data.segmentManager->Segment (m_info.nChild);
}
...
{ 
return g_data.wallManager->Wall (m_info.nWall);
}
...
{ 
CWall* pWall = g_data.wallManager->Wall (m_info.nWall);
...
{
return (Shape () <= SIDE_SHAPE_TRIANGLE) && (((short) m_info.nChild == -1) || g_data.wallManager->IsVisible (m_info.nWall));
}
...
if ((m_info.nWall >= 0) && (m_info.nWall < NO_WALL)) {
	g_data.wallManager->Delete (m_info.nWall);
...
double scale = g_data.textureManager->Scale (g_data.FileType (), BaseTex ());
...
{
return (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [(nIndex + VertexCount ()) % VertexCount ()]]];
}
...
if (Shape () > SIDE_SHAPE_TRIANGLE)
	...
	CDoubleVector& v0 = (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [0]]].m_view;
	CDoubleVector v = vCenter - v0;
	::Normal (m_vNormal [0], v0, (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [1]]].m_view, (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [2]]].m_view);
	...
	if (VertexCount () < 4) 
		...
		::Normal (m_vNormal [1], v0, (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [2]]].m_view, (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [3]]].m_view);
		if (Dot (m_vNormal [1], v) < 0.0)
			m_vNormal [1].Negate ();
		}
	}
else {
	CDoubleVector& v0 = (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [0]]];
	CDoubleVector v = vCenter - v0;
	::Normal (m_vNormal [0], v0, (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [1]]], (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [2]]]);
	if (Dot (m_vNormal [0], v) < 0.0) // make sure normal points into the segment
		m_vNormal [0].Negate ();
	...
	if (VertexCount () < 4) 
		...
		::Normal (m_vNormal [1], v0, (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [2]]], (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [3]]]);
		if (Dot (m_vNormal [1], v) < 0.0)
			m_vNormal [1].Negate ();
		}
	}
...
if (bView) {
	for (short nVertex = 0; nVertex < nVertices; nVertex++) {
		vertices [nVertex] = &(*g_data.vertexManager) [vertexIds [m_vertexIdIndex [nVertex]]].m_view;
	...
	for (short nVertex = 0; nVertex < nVertices; nVertex++) {
		vertices [nVertex] = &(*g_data.vertexManager) [vertexIds [m_vertexIdIndex [nVertex]]];
		if (*p2 == *vertices [nVertex])
			return Distance (*p1, *p2);
		}
	}
...
int h = VertexCount ();
for (int j = 0; j < h; j++) {
	sideVerts [j] = (*g_data.vertexManager) [vertexIds [m_vertexIdIndex [j]]];
...
CVertex* v1, * v2 = &(*g_data.vertexManager) [vertexIds [m_vertexIdIndex [0]]];
for (short nEdge = 1; nEdge <= nEdges; nEdge++) {
	v1 = v2;
	if (!v1->InRange (viewport.right, viewport.bottom, 1))
		continue;
	v2 = &(*g_data.vertexManager) [vertexIds [m_vertexIdIndex [nEdge % nEdges]]];
