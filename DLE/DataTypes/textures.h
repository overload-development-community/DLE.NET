...
public:
	...
	void Read (IFileManager* fp, int nVersion = -1) {
		if (nVersion >= 0)
			m_nVersion = nVersion;
...
class CTexture {
	...
	public:
		...
		int LoadFromPig (IFileManager& fp, uint nIndex, int nVersion = -1);

		bool LoadFromPog (IFileManager& fp, CPigTexture& info);
	...
	private:
		...
		void Load (IFileManager& fp, CPigTexture& info);

		bool LoadTGA (IFileManager& fp);
