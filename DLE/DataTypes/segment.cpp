...
{
if ((m_info.function == 2) && g_data.LevelType () < 2) // standard level repair center type - not supported by standard Descent
	m_info.function = 0;
...
ubyte CSegment::ReadWalls (IFileManager* fp)
{
...
for (i = 0; i < MAX_SIDES_PER_SEGMENT; i++) 
	if (m_info.wallFlags & (1 << i)) 
		m_sides [i].m_info.nWall = (g_data.LevelVersion () >= 13) ? fp->ReadUInt16 () : (ushort) fp->ReadUByte ();
return wallFlags;
}

// -----------------------------------------------------------------------------

void CSegment::ReadExtras (IFileManager* fp, bool bExtras)
{
if (bExtras) {
	m_info.function = fp->ReadSByte ();
	if ((g_data.LevelType () == 2) && (g_data.LevelVersion () > 23)) {
...
m_info.staticLight = (g_data.LevelType () == 0) ? ((int) fp->ReadUInt16 ()) << 4 : fp->ReadInt32 ();
}

// -----------------------------------------------------------------------------

void CSegment::Read (IFileManager* fp)
{
	int	i;

if (g_data.LevelType () == 2) {
...
ubyte CSegment::WriteWalls (IFileManager* fp)
{
	int	i;
...
void CSegment::WriteExtras (IFileManager* fp, bool bExtras)
{
if (bExtras) {
...
void CSegment::Write (IFileManager* fp)
{
	int	i;
...
for (i = 0; i < pSide->VertexCount(); i++)
	A [i] = CDoubleVector (*g_data.vertexManager->Vertex (m_info.vertexIds [pSide->VertexIdIndex (i)])); 
...
CUVL *uvls = m_sides [nSide].m_info.uvls;
g_data.undoManager->Begin (__FUNCTION__, udSegments); 
m_sides [nSide].LoadTextures ();
...
{
	CSegment* pOther = g_data.segmentManager->Segment (nOtherSeg);
	short nCommon = 0;
...
{
return g_data.vertexManager->Vertex (m_info.vertexIds [nVertex]);
}
...
{
return g_data.vertexManager->Vertex (VertexId (nSide, nIndex));
}
...
{ 
return (short (m_sides [nSide].m_info.nChild) < 0) ? null : g_data.segmentManager->Segment (m_sides [nSide].m_info.nChild);
}
...
switch (EditType ()) {
	case opAdd:
		g_data.segmentManager->Remove (Index ());
		break;
	case opDelete:
		g_data.segmentManager->Add ();
		// fall through
	case opModify:
		*GetParent () = *this;
		break;
...
if (nSide < 0) {
	for (int i = 0; i < 8; i++)
		if (m_info.vertexIds [i] <= MAX_VERTEX)
			(*g_data.vertexManager) [m_info.vertexIds [i]].Tag (mask);
	}
else {
	CSide* pSide = Side (nSide);
	for (int i = 0, j = pSide->VertexCount (); i < j; i++)
		(*g_data.vertexManager) [m_info.vertexIds [pSide->VertexIdIndex (i)]].Tag (mask);
	}
}
...
if (nSide < 0) {
	for (int i = 0; i < 8; i++)
		if (m_info.vertexIds [i] <= MAX_VERTEX)
			(*g_data.vertexManager) [m_info.vertexIds [i]].UnTag (mask);
	}
else {
	CSide* pSide = Side (nSide);
	for (int i = 0, j = pSide->VertexCount (); i < j; i++)
		(*g_data.vertexManager) [m_info.vertexIds [pSide->VertexIdIndex (i)]].UnTag (mask);
	}
}
...
if (nSide < 0) {
	for (int i = 0; i < 8; i++)
		if ((m_info.vertexIds [i] <= MAX_VERTEX) && (*g_data.vertexManager) [m_info.vertexIds [i]].IsTagged (mask))
			return true;
	}
else {
	CSide* pSide = Side (nSide);
	for (int i = 0, j = pSide->VertexCount (); i < j; i++)
		if ((*g_data.vertexManager) [m_info.vertexIds [pSide->VertexIdIndex (i)]].IsTagged (mask))
			return true;
	}
return false;
}
...
for (int i = 0; i < 8; i++) {
	if (m_info.vertexIds [i] <= MAX_VERTEX) {
		nVertices++;
		m_vCenter += bView ? (*g_data.vertexManager) [m_info.vertexIds [i]].m_view : (*g_data.vertexManager) [m_info.vertexIds [i]];
		}
	}
...
void CObjectProducer::Read (IFileManager* fp, bool bFlag)
{
...
void CObjectProducer::Write (IFileManager* fp, bool bFlag)
{
