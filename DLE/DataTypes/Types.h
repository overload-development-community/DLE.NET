...
class CSideKey {
	public:
		...
		CSideKey (const CSideKey& other) : m_nSegment (other.m_nSegment), m_nSide (other.m_nSide) {}

		inline bool operator == (const CSideKey& other) { return (m_nSegment == other.m_nSegment) && (m_nSide == other.m_nSide); }
	
		inline bool operator != (const CSideKey& other) { return (m_nSegment != other.m_nSegment) || (m_nSide != other.m_nSide); }
	
		inline bool operator < (const CSideKey& other) { return (m_nSegment < other.m_nSegment) || ((m_nSegment == other.m_nSegment) && (m_nSide < other.m_nSide)); }
...
class CColor : public CGameItem {
	public:
		...
		void Read (IFileManager& fp, bool bFlag = false);

		void Write (IFileManager& fp, bool bFlag = false);
...
class CLevelHeader {
	...
	inline int Read (IFileManager* fp) { return m_header ? m_header->Read (fp) : 0; }
	inline int Write (IFileManager* fp) { return m_header ? m_header->Write (fp) : 0; }
	};
...
class CLevelHeader {
	...
	public:
		...
		int Read (IFileManager* fp);
		int Write (IFileManager* fp, int bExtended = -1);
	};
