...
CRendererSW::CRendererSW (CRenderData& renderData) : CRenderer (renderData), m_DIB (0), m_hThisDC(nullptr)
{
}

//------------------------------------------------------------------------------

void CRendererSW::Reset (const CDoubleVector& currentSegmentCenter)
{
CRenderer::Reset ();
...
int CRendererSW::Setup(HWND hParent, HDC hDC)
{
    ...
    {
        if (!m_hThisDC)
            m_hThisDC = ::CreateCompatibleDC(hDC);
        if (m_hThisDC)
        {
...
void CRendererSW::EndRender(bool bSwapBuffers)
{
    if (bSwapBuffers && (m_DIB != 0))
        ::BitBlt(m_hViewDC, 0, 0, ViewWidth(), ViewHeight(), m_hDC, 0, 0, SRCCOPY);
}
