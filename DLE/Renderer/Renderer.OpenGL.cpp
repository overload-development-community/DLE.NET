...
int CRendererGL::Setup(HWND hParent, HDC hDC)
{
...
GetClientRect (m_hParent, Viewport ());
if (!(Viewport ().Width () && Viewport ().Height ()))
    return 0;
glViewport (Viewport ().left, Viewport ().top, Viewport ().Width (), Viewport ().Height ());
m_glAspectRatio = (GLfloat) Viewport ().Width () / (GLfloat) Viewport ().Height ();
SetupProjection ();
GLTextureHelpers::CreateGLTextures ();
return -1;
}

//------------------------------------------------------------------------------

void CRendererGL::Reset (const CDoubleVector& currentSegmentCenter)
{
CRenderer::Reset ();
m_vZoom.Clear ();
...
if (Perspective ()) {
    Rotation ().Clear ();
    Translation () = -currentSegmentCenter/*current->Segment ()->Center ()*/;
    }
...
GLvoid CRendererGL::DestroyContext (void)
{
    if (m_glRC)
    {
        if (!wglMakeCurrent(null, null))
            g_data.Trace(Error, "OpenGL: Release of DC and RC failed.");
        if (!wglDeleteContext(m_glRC))
            g_data.Trace(Error, "OpenGL: Release of rendering context failed.");
        m_glRC = null;
    }
    if (m_glHDC && !::ReleaseDC(m_hParent, m_glHDC))
    {
    ...
    {
        g_data.Trace(Error, "OpenGL: Could Not Unregister Class.");
    }
#endif
}
...
BOOL CRendererGL::CreateContext (void)
{
    ...
    if (!(m_glHDC = ::GetDC(m_hParent))) {
        DestroyContext();
        g_data.Trace(Error, "OpenGL: Can't create device context.");
        return FALSE;
    }
...
if (textureManager.IsScrolling (pSide->BaseTex ()) && !(bArrow = textureManager.HaveArrow ())) {
    ...
    GLTextureHelpers::GLCreateTexture(&tex, true);
    }
else {
...
if (pTexture[0] == &tex)
    GLTextureHelpers::GLReleaseTexture(&tex);
}
...
if (pColor) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (!GLTextureHelpers::GLBindTexture(pTexture [i], GL_TEXTURE0 + h, GL_MODULATE))
            return;
...
if (!pTexture) {
    ...
    GLTextureHelpers::GLBindTexture (pTexture, GL_TEXTURE0, GL_MODULATE);
