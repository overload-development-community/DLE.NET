...
void CSegmentManager::JoinPoints(ISelection* from, ISelection* to)
{
    CSegment* seg1, * seg2;
    ...
    if (from->SegmentId() == to->SegmentId()) {
        g_data.Trace(Error, "You cannot join two points on the same segment.\n\n"
    ...
    seg1 = Segment(from->SegmentId());
    seg2 = Segment(to->SegmentId());
    vert1 = seg1->m_info.vertexIds[from->Side()->VertexIdIndex(from->Point())];
    vert2 = seg2->m_info.vertexIds[to->Side()->VertexIdIndex(to->Point())];
    // make sure verts are different
    if (vert1 == vert2) {
        g_data.Trace(Error, "These points are already joined.");
        return;
    }
    ...
    if (distance > MAX_JOIN_DISTANCE) {
        g_data.Trace(Error, "Points are too far apart to join.");
        return;
    }
    undoManager.Begin(__FUNCTION__, udSegments);
    // define vert numbers
    seg1->m_info.vertexIds[from->Side()->VertexIdIndex(from->Point())] = vert2;
...
void CSegmentManager::JoinLines(ISelection* from, ISelection* to)
{
    CSegment* seg1, * seg2;
    ...
    if (from->SegmentId() == to->SegmentId()) {
        g_data.Trace(Error, "You cannot join two lines on the same segment.\n\n"
    ...
    for (i = 0; i < 2; i++) {
        i1[i] = seg1->VertexId(from->SideId(), from->Edge() + i);
        i2[i] = seg2->VertexId(to->SideId(), to->Edge() + i);
        v1[i] = seg1->Vertex(from->SideId(), from->Edge() + i);
        v2[i] = seg2->Vertex(to->SideId(), to->Edge() + i);
        match[i] = -1;
    }

    // make sure verts are different
    if ((i1[0] == i2[0]) || (i1[0] == i2[1]) || (i1[1] == i2[0]) || (i1[1] == i2[1])) {
        g_data.Trace(Error, "Some or all of these points are already joined.");
        return;
    }
    ...
    if ((match[0] < 0) || (match[1] < 0)) {
        g_data.Trace(Error, "Lines are too far apart to join.");
        return;
    }
    ...
    // define vert numbers
    for (i = 0; i < 2; i++)
        seg1->SetVertexId(from->SideId(), from->Edge() + i, i2[match[i]]);
    FixChildren(from);
    undoManager.End(__FUNCTION__);
    SetLinesToDraw();
    g_data.RefreshMineView();
...
bool CSegmentManager::FindNearbySide(ISelection* thisSide, CSideKey& otherKey, short& otherPoint, tVertMatch* match)
{
    ...
    sideMatcher.SetSide(0, *thisSide);

    int nSegments = segmentManager.Count();
    for (int nSegment = 0; nSegment < nSegments; nSegment++) {
        if (nSegment == thisSide->SegmentId())
            continue;
    ...
    if (sideMatcher.GetBestMatch(otherKey, otherPoint, match))
        return true;
    g_data.Trace(Warning, "Could not find any other segment's side that is within\n10.0 units from the current side");
    return false;
}
...
void CSegmentManager::Join(ISelection* from, ISelection* to)
{
    CSideMatcher sideMatcher;
    tVertMatch match[4];
    CSideKey otherKey;
    ...
    if (tunnelMaker.Active())
        return;
    if (from->Side()->Child() != nullptr) {
        g_data.Trace(Warning, "The current side is already joined to another segment");
        return;
    }
    ...
    if (to == nullptr)
    {
        if (!FindNearbySide(from, otherKey, otherPoint, match))
            return;
        bJoin = true;
    }
    else
    {
        ...
        if (from->SegmentId() == to->SegmentId())
        {
            g_data.Trace(Warning, "You cannot join two sides on the same segment.\n\n"
        ...
        else if (to->Side()->Child() != nullptr)
        {
            g_data.Trace(Warning, "The selected side of the 'other' segment\n"
                "is already joined to another segment.");
            return;
        }

        sideMatcher.SetSide(0, *from);
        sideMatcher.SetSide(1, otherKey);
        sideMatcher.Match(double(MAX_JOIN_DISTANCE));
        if (!sideMatcher.GetBestMatch(otherKey, otherPoint, match)) {
            g_data.Trace(Warning, "Sides are too far apart to join.\n\n"
    ...
    if (bJoin)
    {
        LinkSides(from->SegmentId(), from->SideId(), otherKey.m_nSegment, otherKey.m_nSide, match);
    }
    else
    {
        CSideKey thisKey(*from);
        thisKey.m_nSegment = Create(thisKey, EXTEND);
        if (thisKey.m_nSegment < 0)
        {
            g_data.Trace(Error, "The maximum number of segments has been reached.\nCannot add any more segments.");
...
void CSegmentManager::JoinSegments(ISelection* atSide, ISelection* toSide)
{
    CSegment* pCurrent = atSide->Segment();
    CSegment* pOther = toSide->Segment();
    ...
    for (short nSide = 0; nSide < MAX_SIDES_PER_SEGMENT; nSide++) {
        if (pCurrent->Side(nSide)->Child() == toSide->Segment()) {
    ...
    if (pJoinSideCurrent == null || pJoinSideOther == null) {
        bValid = false;
        g_data.Trace(Warning, "The selected cube and 'other' cube must be adjacent to be joined.\n\n"
    ...
    if (pOppositeSideCurrent->VertexCount() < 3 ||
        ...
        bValid = false;
    if (!bValid) {
        g_data.Trace(Warning, "Cannot join these cubes because the resulting cube would have illegal geometry.");
        return;
    }
    ...
    // changes back.
    CSideKey currentSideKey(atSide->SegmentId(), pCurrent->SideIndex(pJoinSideCurrent));
    ...
    if (pOppositeSideOther->Child() != null) {
        ...
        if (nSide == 6) {
            g_data.Trace(Error, "Error: Other cube is not correctly joined to its neighboring cubes.\n"
        ...
        joinSideKey = CSideKey(toSide->SegmentId(), pOther->SideIndex(pOppositeSideOther));
    }
    CSideMatcher sideMatcher;
    tVertMatch match[4];
    ...
    if (!sideMatcher.GetBestMatch(joinSideKey, joinPoint, match)) {
        g_data.Trace(Warning, "Error: Unable to join the current cube to the opposite side.");
        return;
    }
...
void CSegmentManager::FixChildren(ISelection* atSide)
{
    short nNewSeg = atSide->SegmentId();
    short nNewSide = atSide->SideId();
