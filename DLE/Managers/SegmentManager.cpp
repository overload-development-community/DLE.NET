...
CSide* CSegmentManager::BackSide(CSideKey key, CSideKey& back)
{
...
void CSegmentManager::CopyOtherSegment(short fromSegmentId, short toSegmentId)
{
    bool bChange = false;

    if (fromSegmentId == toSegmentId)
        return;
    CSegment* pSegment = Segment(toSegmentId);
    CSegment* pOtherSeg = Segment(fromSegmentId);
    undoManager.Begin(__FUNCTION__, udSegments);
    for (int nSide = 0; nSide < 6; nSide++)
        if (SetTextures(CSideKey(toSegmentId, nSide), pOtherSeg->m_sides[nSide].BaseTex(), pOtherSeg->m_sides[nSide].OvlTex()))
            bChange = true;
    undoManager.End(__FUNCTION__);
    if (bChange)
...
void CSegmentManager::MakePointsParallel(ISelection* atSide)
{
    undoManager.Begin(__FUNCTION__, udVertices);
    if (!vertexManager.HasTaggedVertices())
        atSide->Segment()->MakeCoplanar(atSide->SideId());
    else
    {
        double dist;

        atSide->Segment()->ComputeNormals(atSide->SideId());
        CDoubleVector n = atSide->Side()->Normal();
        CDoubleVector v0 = *atSide->Vertex();

        if (atSide->Segment()->HasTaggedVertices(TAGGED_MASK, atSide->SideId()))
            dist = 0.0;
        else {
            double d = 0.0;
            ushort nTagged = 0;
