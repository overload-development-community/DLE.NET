...
void CSegmentManager::SeparateLines(ISelection* atLine)
{
    if (tunnelMaker.Active())
        return;

    short nLine = atLine->Edge();
    CSegment* pSegment = atLine->Segment();
    CSide* pSide = atLine->Side();
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        bShared[i] = VertexInUse(nEdgeVerts[i], atLine->SegmentId());
        if (bShared[i])
            verticesRequired++;
    }
    ...
    for (short nSide = 0; nSide < 6; nSide++) {
        CSideKey back, key(atLine->SegmentId(), nSide);
        if (pSegment->HasEdge(nSide, nEdgeVerts[0], nEdgeVerts[1]) && BackSide(key, back)) {
            UnlinkChild(back.m_nSegment, back.m_nSide);
            UnlinkChild(atLine->SegmentId(), nSide);
        }
    }
...
bool CSegmentManager::SeparateSegments(ISelection* atSide, int bSolidify, bool bVerbose)
{
    if (tunnelMaker.Active())
        return false;
    ...
    for (i = 0; i < nVertices; i++) {
        ...
        if ((bShared[i] = VertexInUse(nFaceVerts[i], atSide->SegmentId())))
            ++nShared;
    }
    ...
    if (!bSolidify) {
        ...
        for (int nSide = 0; nSide < 6; nSide++)
            if (nSide != nOppSide)
                UnlinkChild(atSide->SegmentId(), nSide);
        SetLinesToDraw();
        char message[50]{};
        ...
        int nChildSide;
        for (nChildSide = 0; nChildSide < 6; nChildSide++)
            if (pChildSeg->ChildId(nChildSide) == atSide->SegmentId())
                break;
        // if we bShared the matching side
        if (nChildSide < 6)
            ResetSide(nChildSeg, nChildSide);
        ResetSide(atSide->SegmentId(), atSide->SideId());
        SetLinesToDraw();
    }
    undoManager.End(__FUNCTION__);
    g_data.RefreshMineView();
...
bool CSegmentManager::SplitIn7(CSegment* pCenterSeg)
{
...
bool CSegmentManager::CreateWedge(ISelection* atSide)
{
    ...
    short nSide = atSide->SideId(), nOppSide = oppSideTable[atSide->SideId()];

    undoManager.Begin(__FUNCTION__, udSegments | udVertices | udWalls);

    short nLine = atSide->Edge() + 1;
    if (!CollapseEdge(-1, -1, nLine % atSide->Side()->VertexCount())) {
        undoManager.Unroll(__FUNCTION__);
        return false;
    }
    if (nLine < atSide->Side()->VertexCount())
        ++nLine;
    if (!CollapseEdge(-1, -1, nLine % atSide->Side()->VertexCount())) {
        undoManager.Unroll(__FUNCTION__);
        return false;
...
bool CSegmentManager::CreatePyramid(ISelection* atSide)
{
    ...
    if (nLine < atSide->Side()->VertexCount())
        ++nLine;
    if (!(CollapseEdge(-1, -1, nLine % atSide->Side()->VertexCount()) && CollapseEdge(-1, -1, 0))) {
        undoManager.Unroll(__FUNCTION__);
        return false;
    }
