...
void CBlockManager::SetupTransformation(ISelection* atSide, CDoubleMatrix& t, CDoubleVector& o)
{
    CSegment* pSegment = atSide->Segment();
    o = *pSegment->Vertex(atSide->SideId(), atSide->Point());
    // set x'
    t.m.rVec = *pSegment->Vertex(atSide->SideId(), atSide->Point() + 1);
    t.m.rVec -= o;
    // calculate y'
    CDoubleVector v = *pSegment->Vertex(atSide->SideId(),
        atSide->Point() + atSide->Side()->VertexCount() - 1);
    v -= o;
...
short CBlockManager::Read(ISelection* destSide, CFileManager& fp)
{
    int				i, j, scanRes;
    short				origVertCount;
    CDoubleMatrix	t, r;
    ...
    // set origin
    SetupTransformation(destSide, t, origin);
    ...
    while (!fp.EoF()) {
        ...
        for (short nSide = 0; nSide < MAX_SIDES_PER_SEGMENT; nSide++, pSide++) {
            ...
            if (m_bExtended) {
                ...
                if (pSide->m_info.nWall != NO_WALL) {
                    ...
                    CSideKey wallKey(nSegment, nSide);
                    if (wallManager.HaveResources(wallKey)) {
                        if ((w.Info().nTrigger >= 0) && (w.Info().nTrigger < MAX_TRIGGERS)) {
                            if (!triggerManager.HaveResources(wallKey))
                                w.Info().nTrigger = NO_TRIGGER;
                            else {
                                w.Info().nTrigger = (ubyte)triggerManager.Add(wallKey);
        ...
        if (m_bExtended) {
            ...
            case SEGMENT_FUNC_PRODUCER:
                if (!segmentManager.CreateProducer(CSideKey(), nSegment, SEGMENT_FUNC_PRODUCER, false, false))
                    pSegment->m_info.function = 0;
                break;
            case SEGMENT_FUNC_REPAIRCEN:
                if (!segmentManager.CreateProducer(CSideKey(), nSegment, SEGMENT_FUNC_REPAIRCEN, false, false))
                    pSegment->m_info.function = 0;
                break;
            case SEGMENT_FUNC_ROBOTMAKER:
                if (!segmentManager.CreateRobotMaker(CSideKey(), nSegment, false, false))
                    pSegment->m_info.function = 0;
                break;
            case SEGMENT_FUNC_EQUIPMAKER:
                if (!segmentManager.CreateEquipMaker(CSideKey(), nSegment, false, false))
                    pSegment->m_info.function = 0;
                break;
            case SEGMENT_FUNC_REACTOR:
                if (!segmentManager.CreateReactor(CSideKey(), nSegment, false, false))
                    pSegment->m_info.function = 0;
                break;
            default:
                break;
...
void CBlockManager::Write(ISelection* sourceSide, CFileManager& fp)
{
    short				j;
    CVertex			origin;
    CDoubleMatrix	m;
    ...
    // set origin
    SetupTransformation(sourceSide, m, origin);
...
void CBlockManager::Copy(ISelection* sourceSide, const char* filename, bool bDelete)
{
...
void CBlockManager::Cut(ISelection* sourceSide, const char* filename)
{
    Copy(sourceSide, filename, true);
}

//------------------------------------------------------------------------------

int CBlockManager::Read(ISelection* destSide, char* filename)
{
    CFileManager fp;
...
void CBlockManager::QuickCopy(ISelection* sourceSide)
{
    Copy(sourceSide, "dle_temp.blx");
}
