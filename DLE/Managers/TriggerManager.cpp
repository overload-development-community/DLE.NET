...
short CTriggerManager::Add(CSideKey key)
{
...
CTrigger* CTriggerManager::AddToWall(CSideKey key, short type, bool bAddWall)
{
    ...
    if (segmentManager.Trigger(key) != null) {
        g_data.Trace(Error, "There is already a trigger on this side");
        return null;
    }
    if (Full()) {
        g_data.Trace(Error, "The maximum number of triggers has been reached.");
        return null;
    }
    // if no wall at current side, try to add a wall of proper type
    undoManager.Begin(__FUNCTION__, udTriggers);

    auto side = segmentManager.Side(key);
    CWall* pWall = side->Wall();

    if (pWall == null) {
        if (bAddWall) {
            if (wallManager.Count() >= MAX_WALLS) {
                g_data.Trace(Error, "Cannot add a wall for this trigger\nsince the maximum number of walls is already reached.");
                undoManager.Unroll(__FUNCTION__);
                return null;
            }
            pWall = wallManager.Create(CSideKey(), (side->Child() == nullptr) ? WALL_OVERLAY : defWallTypes[type], 0, 0, -1, defWallTextures[type]);
            if (pWall == null) {
                g_data.Trace(Error, "Cannot add a wall for this trigger.");
                undoManager.Unroll(__FUNCTION__);
                return null;
            ...
            g_data.Trace(Error, "You need to add a wall to this side before you can add a trigger.");
            undoManager.Unroll(__FUNCTION__);
            return null;
        }
    }
...
void CTriggerManager::DeleteFromWall(CSideKey key)
{
    auto side = segmentManager.Side(key);
    CWall* pWall = side->Wall();
    if (pWall == null)
        return;
    short nDelTrigger = pWall->Info().nTrigger;
...
CTrigger* CTriggerManager::AddToObject(short nObject, short type)
{
    ...
    if (pObject == null) {
        g_data.Trace(Error, "Couldn't find object to attach triggers to.");
        return null;
    }
    ...
    if ((pObject->Info().type != OBJ_ROBOT) &&
        ...
        g_data.Trace(Error, "Object triggers can only be attached to robots, reactors, hostages, powerups and cameras.");
        return null;
    }

    if (ObjTriggerCount() >= MAX_OBJ_TRIGGERS) {
        g_data.Trace(Error, "The maximum number of object triggers has been reached.");
        return null;
    }
...
bool CTriggerManager::HaveResources(CSideKey key)
{
    if (!wallManager.HaveResources(key))
        return false;
    if (Full()) {
        g_data.Trace(Error, "Maximum number of triggers reached");
        return false;
    }
    return true;
}
...
bool CTriggerManager::AutoAddTrigger(CSideKey source, CSideKey target, short wallType, ushort wallFlags, ushort triggerType)
{
    if (!HaveResources(source))
        return false;
    // make a new wall and a new trigger
    undoManager.Begin(__FUNCTION__, udTriggers);
    CWall* pWall = wallManager.Create(source, (ubyte)wallType, wallFlags, KEY_NONE, -1, -1);
    if (pWall != null) {
...
bool CTriggerManager::AddDoorTrigger(CSideKey source, CSideKey target, short wallType, ushort wallFlags, ushort triggerType)
{
    if (segmentManager.Side(target)->Wall() == null)
    {
        g_data.Trace(Error, "Other segment's side is not on a wall.\n\n"
    ...
    if (segmentManager.Side(target)->Wall()->Info().type != WALL_DOOR)
        triggerType = TT_OPEN_WALL;
    return AutoAddTrigger(source, target, wallType, wallFlags, triggerType);
}

// -----------------------------------------------------------------------------

bool CTriggerManager::AddOpenDoor(CSideKey source, CSideKey target)
{
    return AddDoorTrigger(source, target, WALL_OPEN, 0, TT_OPEN_DOOR);
}

// -----------------------------------------------------------------------------

bool CTriggerManager::AddRobotMaker(CSideKey source, CSideKey target)
{
    if (segmentManager.Segment(target)->Info().function != SEGMENT_FUNC_ROBOTMAKER)
    {
        g_data.Trace(Error, "There is no robot maker segment selected.\n\n"
            "Hint: Select a robot maker segment using the 'other segment' and\n"
            "select a trigger location using the 'current segment'.");
        return false;
    }
    return AutoAddTrigger(source, target, WALL_OPEN, 0, TT_MATCEN);
}

// -----------------------------------------------------------------------------

bool CTriggerManager::AddShieldDrain(CSideKey source, CSideKey target)
{
    if (g_data.IsD2File())
    {
        g_data.Trace(Error, "Descent 2 does not support shield damage Triggers ()");
        return false;
    }
    return AutoAddTrigger(source, target, WALL_OPEN, 0, TT_SHIELD_DAMAGE);
}

// -----------------------------------------------------------------------------

bool CTriggerManager::AddEnergyDrain(CSideKey source, CSideKey target)
{
    if (g_data.IsD2File())
    {
        g_data.Trace(Error, "Descent 2 does not support energy drain Triggers ()");
        return false;
    }
    return AutoAddTrigger(source, target, WALL_OPEN, 0, TT_ENERGY_DRAIN);
}

// -----------------------------------------------------------------------------

bool CTriggerManager::AddUnlock(CSideKey source, CSideKey target)
{
    if (g_data.IsD1File())
    {
        g_data.Trace(Error, "Control Panels are not supported in Descent 1.");
        return false;
    }
    return AddDoorTrigger(source, target, WALL_OVERLAY, WALL_WALL_SWITCH, TT_UNLOCK_DOOR);
}
